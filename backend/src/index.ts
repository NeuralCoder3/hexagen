...
import { canUserGenerateImage, recordImageGeneration, startActiveGeneration, stopActiveGeneration, isCoordinateBeingGenerated } from './utils/rateLimiter';
import { recordImageGeneration as recordGeneration, getUpdatedCoordinates, getGenerationTimestamps, getLastSystemUpdate } from './utils/generationTracker';
...
// Check if tile can be generated here (adjacency rule + rate limiting)
app.get('/api/hexagon/:x/:y/can-generate', (req, res) => {
  ...
  if (!username) {
    return res.status(401).json({ allowed: false, reason: 'Authentication required' });
  }
  
  const rateLimitCheck = canUserGenerateImage(username, { x: xNum, y: yNum });
  if (!rateLimitCheck.allowed) {
    return res.json({ 
      allowed: false, 
      reason: rateLimitCheck.reason,
      timeUntilNext: rateLimitCheck.timeUntilNext,
      isGenerating: rateLimitCheck.isGenerating,
      generatingCoordinates: rateLimitCheck.generatingCoordinates
    });
  }
  
  // Check if this specific coordinate is being generated by any user
  const coordinateCheck = isCoordinateBeingGenerated({ x: xNum, y: yNum });
  if (coordinateCheck.isGenerating) {
    return res.json({
      allowed: false,
      reason: `Tile at (${xNum}, ${yNum}) is currently being generated by ${coordinateCheck.generatingUser}. Please wait for it to complete.`,
      isGenerating: true,
      generatingUser: coordinateCheck.generatingUser
    });
  }
...
app.post('/api/generate-tile', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Check rate limiting (security check)
    ...
    if (coordinateCheck.isGenerating) {
      return res.status(409).json({
        error: `Tile at (${xNum}, ${yNum}) is currently being generated by ${coordinateCheck.generatingUser}. Please wait for it to complete.`,
        isGenerating: true,
        generatingUser: coordinateCheck.generatingUser
      });
    }
    ...
    startActiveGeneration(username, { x: xNum, y: yNum });
    ...
    permanentImagePath = path.join(paths.coordinateImages, `coordinate_${xNum}_${yNum}_${timestamp}_${randomId}.png`);
    ...
    formData.append('image[]', imageBuffer, path.basename(jpgImagePath));
    ...
    console.log('- image file:', path.basename(jpgImagePath));
    ...
    if (data.data && data.data[0] && data.data[0].url) {
      ...
      recordImageGeneration(username);
      
      // Record generation in tracker for frontend updates
      recordGeneration(xNum, yNum, username);
      
      // Stop tracking active generation
      stopActiveGeneration(username);
    ...
    // Stop tracking active generation on error
    if (username) {
      stopActiveGeneration(username);
    }
...
// Check for updated images since a given timestamp
app.get('/api/updates', (req, res) => {
  try {
    const sinceTimestamp = parseInt(req.query.since as string) || 0;
    const updatedCoordinates = getUpdatedCoordinates(sinceTimestamp);
    const lastSystemUpdate = getLastSystemUpdate();
    
    res.json({
      updated: updatedCoordinates,
      lastSystemUpdate,
      hasUpdates: updatedCoordinates.length > 0
    });
  } catch (error) {
...
// Get generation timestamps for specific coordinates
app.post('/api/generation-timestamps', (req, res) => {
  try {
    const { coordinates } = req.body;
    
    if (!Array.isArray(coordinates)) {
      return res.status(400).json({ error: 'coordinates must be an array of {x, y}' });
    }
    ...
    if (validCoords.length === 0) {
      return res.status(400).json({ error: 'No valid coordinates provided' });
    }
    
    const timestamps = getGenerationTimestamps(validCoords);
    const lastSystemUpdate = getLastSystemUpdate();
    ...
    console.error('Error getting generation timestamps:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
